...
- **Link every ticker to its Yahoo Finance quote page.** Render each ticker symbol as `[TICKER](https://finance.yahoo.com/quote/TICKER)` — e.g. `[GRID](https://finance.yahoo.com/quote/GRID)`. Yahoo is the default review destination: most popular, free, no login, and one URL pattern covers both stocks and ETFs.
  ...
  - Link tickers in their **anchor locations** — every table cell that holds a ticker (TL;DR, per-realm decision matrices, Final Picks at a Glance) and every heading or ranked-list line that names one (the `### ETF Pick: [TICKER] — ...` headings, the `[TICKER] (XX%) — ...` best-entry bullets). Don't link every prose mention; anchors keep the report clickable without link spam.
  - The link wraps only the symbol text — keep adjacent prices (`\$...`) and percentages as plain text outside it.
